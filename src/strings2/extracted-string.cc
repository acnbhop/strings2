...
CExtractedString::CExtractedString(const char* szString, size_t iSizeInBytes, eStringType eType,
...
CExtractedString::CExtractedString(const wchar_t* szString, size_t iSizeInBytes, eStringType eType,
                                   s32 iOffsetStart, s32 iOffsetEnd)
{
    m_Type = eType;
    ...
    for (usize i = 0; i < iLength; i++)
    {
        ...
        if (m_String[i] >= 0x9 && m_String[i] <= 0x7E)
        {
            ...
            #   pragma GCC diagnostic pop
                ...
                fScore += (f32)string_model::fWeights[118 + (m_String[i] - 0x9) +
                                                      118 * (m_String[i + 1] - 0x9)];
            }
        }
        else
        {
...
std::string CExtractedString::GetString() { return m_String; }

bool CExtractedString::IsInteresting() { return GetProbaInteresting() > 0.5f; }

eStringType CExtractedString::GetType() { return m_Type; }
...
int CExtractedString::GetOffsetStart() { return m_OffsetStart; }

int CExtractedString::GetOffsetEnd() { return m_OffsetEnd; }
