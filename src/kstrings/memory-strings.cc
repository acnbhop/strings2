...
CMemoryStrings::CMemoryStrings(CStringParser* pParser) { this->m_Parser = pParser; }
...
bool CMemoryStrings::DumpProcess(DWORD iPID)
{
    ...
    if (pHandle != NULL)
    {
        ...
        {
            fprintf(stderr, "Failed to gather module information for process 0x%x (%lu). ",
                    (u32)iPID, iPID);
...
sMBIBasicInfo CMemoryStrings::_GetMbiInfo(u64 iAddress, HANDLE pHandle)
{
    ...
    SIZE_T blockSize = VirtualQueryEx(pHandle, (LPCVOID)iAddress, (PMEMORY_BASIC_INFORMATION)&mbi,
    ...
    if (blockSize == sizeof(_MEMORY_BASIC_INFORMATION64))
    {
        ...
        result.bExecutable = (mbi.Protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ |
    ...
    else
    {
        ...
        if (VirtualQueryEx(pHandle, (LPCVOID)iAddress, (PMEMORY_BASIC_INFORMATION)&mbi32,
                           sizeof(_MEMORY_BASIC_INFORMATION32)) ==
            sizeof(_MEMORY_BASIC_INFORMATION32))
        {
            ...
            result.bValid =
                (mbi32.State != MEM_FREE) && !(mbi32.Protect & (PAGE_NOACCESS | PAGE_GUARD));
            result.bExecutable =
                (mbi32.Protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE |
...
bool CMemoryStrings::_ProcessAllMemory(HANDLE pHandle, std::string process_name)
{
    ...
    while (address < max_address)
    {
        ...
        if (mbi_info.bValid && mbi_info.iSize > 0)
        {
            ...
            {
                SIZE_T num_read = 0;
                BOOL result = ReadProcessMemory(pHandle, (LPCVOID)mbi_info.iBase, buffer,
                                                mbi_info.iSize, &num_read);
                ...
                if (result && num_read > 0)
                {
                    ...
                    long_name << process_name << ":" << module_name_long << "@0x" << std::hex
                              << mbi_info.iBase;
                    std::stringstream short_name;
                    short_name << process_name << ":" << module_name_short << "@0x" << std::hex
                               << mbi_info.iBase;

                    m_Parser->ParseBlock(buffer, (u32)num_read, short_name.str(), long_name.str(),
                                         mbi_info.iBase);
                }
                else
                {
