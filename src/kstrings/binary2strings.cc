...
{
    // Returns 0 if it's not likely a valid utf8 character. For ascii range of characters it
    // requires the character to be a displayable character.
    ...
    if (iFirstByte > 0xF4)
    {
        return 0; // Invalid bytes for Utf8,
                                   ...
                                   usize iMinChars)
{
    ...
    if (UtfCharLength > 0)
    {
        ...
        if (iCharCount >= iMinChars)
        {
            ...
            return new CExtractedString((char*)(pBuffer + iOffset), iLocalOffset - iOffset,
                                        eStringType::UTF8, (s32)iOffset, (s32)(iLocalOffset - 1));
        }
    ...
    {
        // Return the extracted string
        return new CExtractedString((wchar_t*)(pBuffer + iOffset), iLocalOffset - iOffset,
                                    eStringType::WideString, (s32)iOffset, (s32)(iLocalOffset - 2));
    }
    ...
    if (s != NULL)
    {
        ...
        if (!only_interesting || is_interesting)
        {
            ...
            auto result = std::make_tuple(
                s->GetString(), s->GetTypeString(),
                std::pair<s32, s32>(s->GetOffsetStart(), s->GetOffsetEnd()), is_interesting);
...
ExtractAllStrings(const u8 pBuffer[], usize iBufferSize, usize iMinChars, bool only_interesting)
{
    ...
    while (iOffset + iMinChars <= iBufferSize)
    {
        ...
        if (s)
        {
            ...
            // Add the new string
            r_vect.push_back(std::tuple<std::string, std::string, std::pair<s32, s32>, bool>(
                s->GetString(), s->GetTypeString(),
...
#pragma GCC diagnostic pop
        ...
        {
            r_vect_filt.push_back(std::tuple<std::string, std::string, std::pair<s32, s32>, bool>(
                std::get<0>(r_vect[i]), std::get<1>(r_vect[i]), std::get<2>(r_vect[i]),
